...
void SourceTerms::CGMCooling(const DvceArray5D<Real> &w0, const EOS_Data &eos_data,
  ...
  KOKKOS_LAMBDA(const int m, const int k, const int j, const int i) {
    ...
    Real gamma_heating = h_rate * h_norm * X * nH_unit;
    if (h_flag) {
      // Fall back to uniform heating when the profile parameters are not usable.
      const Real height_sq = h_height * h_height;
      if (h_radius > 0.0 && height_sq > 0.0) {
        ...
        const Real vert_scale2 = fma(h_alpha, R2, height_sq);
        if (vert_scale2 > 0.0) {
          const Real vert_falloff = exp(-(x3v*x3v) / vert_scale2);
          gamma_heating *= horz_falloff * vert_falloff;
        }
      }
    }
