...
v_turb = np.sqrt(np.average(v1**2))
...
v_arr = np.linspace(0, 5, num=100)
plt.plot(
    v_arr,
    maxwell.cdf(v_arr, scale= v_turb*np.sqrt(2/3)),
    alpha=0.8,
    label="Maxwell CDF",
)
...
plt.figure()
v_shatt = v_rel
v_shatt[v_shatt < 1] = None
plt.hist(
    v_shatt, bins=100, density=True, alpha=0.2, label="$|v_1-v_2|>v0$"
)
plt.show()